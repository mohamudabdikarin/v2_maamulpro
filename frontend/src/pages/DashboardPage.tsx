...
const TenantFinancialChart = ({ metrics }: { metrics: TenantExecutiveMetrics }) => {
    ...
    const options: any = {
        chart: { type: 'area', fontFamily: 'Nunito, sans-serif', toolbar: { show: false }, zoom: { enabled: false }, background: 'transparent' },
...
const TenantModuleDistribution = ({ metrics }: { metrics: TenantExecutiveMetrics }) => {
    ...
    const options: any = { chart: { type: 'donut', fontFamily: 'Nunito, sans-serif', background: 'transparent' }, labels: data.map((item) => item.label), colors: data.map((item) => item.color), dataLabels: { enabled: false }, stroke: { width: 5, colors: [isDark ? '#0e1726' : '#fff'] }, legend: { position: 'bottom', labels: { colors: isDark ? '#bfc9d4' : '#0e1726' } }, plotOptions: { pie: { donut: { size: '68%', labels: { show: true, total: { show: true, label: 'Records', color: '#888ea8', formatter: (context: any) => context.globals.seriesTotals.reduce((sum: number, value: number) => sum + value, 0) } } } } }, theme: { mode: isDark ? 'dark' : 'light' } };
