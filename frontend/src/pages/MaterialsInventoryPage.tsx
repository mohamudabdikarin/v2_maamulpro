...
const MaterialsInventoryPage = () => {
    const state = useApiRows<any>('/api/materials/products'); const [search, setSearch] = useState(''); const [warehouse, setWarehouse] = useState('');
    ...
    return <AppShell><PageHeader eyebrow="Materials inventory" title="Stock catalog" description="Product images, warehouses, buying cost, selling price and replenishment thresholds." actions={<Link className="btn btn-primary" to="/app/materials/inventory/manage">Manage products</Link>} />
        ...
        {state.error && <ErrorAlert message={state.error} onRetry={state.reload} />}{state.loading ? <div className="panel"><LoadingState /></div> : !rows.length ? <div className="panel"><EmptyState title="No matching materials" /></div> : <div className="panel overflow-hidden p-0"><div className="overflow-x-auto"><table className="table-hover"><thead><tr><th>Material</th><th>Warehouse</th><th>On hand</th><th>Unit cost</th><th>Sale price</th><th>Stock value</th><th>Status</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><div className="flex items-center gap-3">{row.photoUrl ? <AuthenticatedImage className="h-10 w-10 rounded object-cover" src={row.photoUrl} alt="" /> : <div className="grid h-10 w-10 place-items-center rounded bg-primary-light text-primary">□</div>}<div><strong>{row.name}</strong><p className="text-xs text-white-dark">{row.category || row.materialType || row.unit}</p></div></div></td><td>{row.warehouse || 'General'}</td><td><span className={Number(row.quantity) <= Number(row.lowStockThreshold) ? 'font-bold text-danger' : 'font-bold'}>{row.quantity} {row.unit}</span></td><td>{money(row.unitCost)}</td><td>{money(row.salePrice)}</td><td>{money(Number(row.quantity) * Number(row.unitCost))}</td><td><StatusPill value={row.status} /></td></tr>)}</tbody></table></div></div>}
    </AppShell>;
};
