...
const SuperAdminCompanyPage = () => {
    ...
    const run = async (key: string, path: string, method: 'POST' | 'PATCH' | 'DELETE' = 'POST', body: unknown = {}, successMessage = 'Action completed successfully.') => {
        ...
        try { await api(path, { method, body: JSON.stringify(body), silent: true }); await load(); toast.success(successMessage); return true; }
        catch (reason) { const msg = reason instanceof Error ? reason.message : 'Action failed.'; toast.error(msg); setError(msg); return false; }
    ...
    const createTemporaryPassword = async () => {
        ...
        try { const result = await api<{ password: string }>(`/api/superadmin/companies/${id}/owner/temporary-password`, { method: 'POST', silent: true }); setTemporaryPassword(result.password); toast.success('Temporary owner password generated. Copy it now; it will not be shown again.', 8000); }
    ...
    const saveProfile = async (event: FormEvent) => {
        ...
        try { await api(`/api/superadmin/companies/${id}`, { method: 'PATCH', body: JSON.stringify(form), silent: true }); setEditing(false); await load(); toast.success('Tenant profile updated.'); }
        catch (reason) { const msg = reason instanceof Error ? reason.message : 'Unable to update tenant profile.'; toast.error(msg); setError(msg); }
        finally { setSaving(false); }
    };
    const configureSubscription = async (event: FormEvent) => {
        event.preventDefault();
        const saved = await run('subscription', `/api/superadmin/companies/${id}/subscription`, 'PATCH', { amount: Number(subscriptionForm.amount), termDurationMonths: Number(subscriptionForm.termDurationMonths), autoRecur: subscriptionForm.autoRecur, notes: subscriptionForm.notes || undefined }, 'Subscription configured.');
