...
const RealEstateOverviewPage = () => {
    ...
    const load = () => Promise.all([read('properties.read', '/api/real-estate/properties'), read('clients.read', '/api/real-estate/clients'), read('deals.read', '/api/real-estate/deals'), read('rentals.read', '/api/real-estate/rental-contracts'), read('rentals.read', '/api/real-estate/rent-payments')])
    ...
    return <AppShell>
        ...
        {!data ? <div className="panel"><LoadingState /></div> : <>
            ...
            <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
                ['Properties', '/app/real-estate/properties', 'Listings and occupancy', 'properties.read'], ['Clients', '/app/real-estate/clients', `${data.clients.length} contacts`, 'clients.read'],
                ['Sales', '/app/real-estate/sales', 'Deals and collections', 'deals.read'], ['Rentals', '/app/real-estate/rentals', 'Tenants, leases and rent', 'rentals.read'],
            ].filter(([, , , permission]) => hasPermission(permission)).map(([title, to, description]) => <Link className="panel transition hover:-translate-y-1 hover:border-primary" to={to} key={to}><h2 className="font-bold text-primary">{title}</h2><p className="mt-1 text-sm text-white-dark">{description}</p></Link>)}</div>
            <div className="grid gap-6 xl:grid-cols-2">
                {hasPermission('deals.read') && <div className="panel overflow-hidden p-0"><div className="flex items-center justify-between p-5"><div><h2 className="text-lg font-bold">Recent deals</h2><p className="text-sm text-white-dark">Latest sales and rental agreements</p></div><Link to="/app/real-estate/deals" className="text-primary">View all</Link></div>{!data.deals.length ? <EmptyState title="No deals yet" /> : <div className="overflow-x-auto"><table className="table-hover"><thead><tr><th>Property</th><th>Client</th><th>Type</th><th>Collected</th><th>Status</th></tr></thead><tbody>{data.deals.slice(0, 8).map((deal) => <tr key={deal.id}><td><Link className="text-primary" to={`/app/real-estate/deals/${deal.id}`}>{deal.property?.title || 'Property'}</Link></td><td>{deal.client?.name}</td><td>{deal.type}</td><td>{money(deal.paidAmount)}</td><td><StatusPill value={deal.paymentStatus} /></td></tr>)}</tbody></table></div>}</div>}
