    ...
    return <AppShell>
        ...
        {!data ? <div className="panel"><LoadingState /></div> : <>
            ...
            <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
                ...
                { title: 'Sales', to: '/app/real-estate/sales', description: 'Deals and collections', visible: hasPermission('deals.read') },
            ...
            <div className="grid gap-6 xl:grid-cols-2">
                {hasPermission('deals.read') && <div className="panel overflow-hidden p-0"><div className="flex items-center justify-between p-5"><div><h2 className="text-lg font-bold">Recent deals</h2><p className="text-sm text-white-dark">Latest sales and rental agreements</p></div><Link to="/app/real-estate/deals" className="text-primary">View all</Link></div>{!data.deals.length ? <EmptyState title="No deals yet" description="Record a property sale or rental deal to start tracking settlement." action={hasPermission('deals.create') ? <Link className="btn btn-primary" to="/app/real-estate/deals/new">Create first deal</Link> : undefined} /> : <div className="overflow-x-auto"><table className="table-hover"><thead><tr><th>Property</th><th>Client</th><th>Type</th><th>Collected</th><th>Status</th></tr></thead><tbody>{data.deals.slice(0, 8).map((deal) => <tr key={deal.id}><td><Link className="text-primary" to={`/app/real-estate/deals/${deal.id}`}>{deal.property?.title || 'Property'}</Link></td><td>{deal.client?.name}</td><td>{deal.type}</td><td>{money(deal.paidAmount)}</td><td><StatusPill value={deal.paymentStatus} /></td></tr>)}</tbody></table></div>}</div>}
