...
const FinancialsPage = () => {
    ...
    return <AppShell>
        <PageHeader eyebrow="Unified ledger" title="Financials" description="Company income, expenses, categorized sources and synchronized operational ledger entries." actions={<>{hasPermission('transactions.read') && <button className="btn btn-outline-primary" onClick={exportCsv}>Export CSV</button>}{hasPermission('transactions.create') && <button className="btn btn-primary" onClick={() => { setEditing(null); setForm(blank); setSourceType('none'); setOpen(true); }}>New transaction</button>}</>} />
