...
import { EmptyState, ErrorAlert, Field, FormActions, LoadingState, Modal, PageHeader, StatGrid, StatusPill, formatDescription, formatReference, money, shortDate } from '../components/maamulpro/PageKit';
...
const blank = { type: 'EXPENSE', status: 'CLEARED', description: '', amount: '', date: '', categoryId: '', projectId: '', propertyId: '', notes: '' };
...
const FinancialsPage = () => {
    ...
    const exportCsv = () => {
        ...
        const body = rows.map((row) => [shortDate(row.date), formatReference(row.referenceId, row.id), row.type, formatDescription(row.description), row.category?.name, row.project?.name || row.property?.title || 'General', row.amount, row.status, row.notes].map(csvCell).join(','));
    ...
    return <AppShell>
        <PageHeader eyebrow="Unified ledger" title="Financials" description="Company income, expenses, categorized sources and synchronized operational ledger entries." actions={<><button className="btn btn-outline-primary" onClick={exportCsv}>Export CSV</button><button className="btn btn-primary" onClick={() => { setEditing(null); setForm(blank); setSourceType('none'); setOpen(true); }}>New transaction</button></>} />
