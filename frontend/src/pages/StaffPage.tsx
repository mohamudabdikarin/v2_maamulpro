...
const StaffPage = () => {
    ...
    const remove = async (row: Staff) => { if (!confirm(`Terminate ${row.firstName} ${row.lastName}?`)) return; try { await api(`/api/staff/${row.id}`, { method: 'DELETE' }); setSelected(null); await state.reload(); } catch (reason) { state.setError(reason instanceof Error ? reason.message : 'Unable to delete staff'); } };
    ...
    const accountAction = async (action: 'status' | 'email' | 'password') => {
        if (!selected?.user) return;
        const value = action === 'status' ? !selected.user.isActive : prompt(action === 'email' ? 'New email address' : 'Temporary password (12+ characters)');
        if (value === null) return;
        const path = action === 'status' ? 'status' : action === 'email' ? 'email' : 'reset-password';
        const body = action === 'status' ? { isActive: value } : action === 'email' ? { email: value } : { temporaryPassword: value };
        try { await api(`/api/staff/${selected.id}/account/${path}`, { method: action === 'password' ? 'POST' : 'PATCH', body: JSON.stringify(body) }); setMessage('Account updated successfully.'); await state.reload(); setSelected(await api<Staff>(`/api/staff/${selected.id}`)); } catch (reason) { state.setError(reason instanceof Error ? reason.message : 'Account update failed'); }
    };
    return <AppShell>
        ...
        <div className="panel mb-5 grid gap-3 md:grid-cols-3"><input className="form-input" placeholder="Search staff…" value={search} onChange={(event) => setSearch(event.target.value)} /><select className="form-select" value={department} onChange={(event) => setDepartment(event.target.value)}><option value="">All departments</option>{['GENERAL', 'CONSTRUCTION', 'REAL_ESTATE', 'MATERIAL_MANAGEMENT'].map((value) => <option value={value}>{value.replace(/_/g, ' ')}</option>)}</select><select className="form-select" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{['ACTIVE', 'INACTIVE', 'ON_LEAVE', 'TERMINATED'].map((value) => <option value={value}>{value.replace(/_/g, ' ')}</option>)}</select></div>
        <div className="panel overflow-hidden p-0">{state.loading ? <LoadingState /> : !filtered.length ? <EmptyState title="No staff members found" action={<button className="btn btn-primary" onClick={openCreate}>Add staff member</button>} /> : <div className="overflow-x-auto"><table className="table-hover w-full"><thead><tr><th>Employee</th><th>Department</th><th>Position</th><th>Status</th><th>Account</th><th>Salary</th><th /></tr></thead><tbody>{filtered.map((row) => <tr key={row.id}><td><button className="flex items-center gap-3 text-left" onClick={() => setSelected(row)}>{row.photoUrl ? <img className="h-10 w-10 rounded-full object-cover" src={row.photoUrl} /> : <span className="grid h-10 w-10 place-items-center rounded-full bg-primary-light font-bold text-primary">{row.firstName[0]}{row.lastName[0]}</span>}<span><strong>{row.firstName} {row.lastName}</strong><small className="block text-white-dark">{row.phone || 'No phone'}</small></span></button></td><td>{row.department.replace(/_/g, ' ')}</td><td>{row.position || '—'}</td><td><StatusPill value={row.status} /></td><td>{row.user ? <StatusPill value={row.user.isActive ? 'ACTIVE' : 'INACTIVE'} /> : <span className="text-white-dark">No login</span>}</td><td>{money(row.salary)}</td><td><div className="flex gap-2"><button className="btn btn-sm btn-outline-primary" onClick={() => openEdit(row)}>Edit</button><button className="btn btn-sm btn-outline-danger" onClick={() => remove(row)}>Terminate</button></div></td></tr>)}</tbody></table></div>}</div>
        <Modal title={editing ? 'Edit staff member' : 'Add staff member'} open={formOpen} onClose={() => setFormOpen(false)} wide><form className="grid gap-4 md:grid-cols-2" onSubmit={save}>
            {form.photoUrl && <img className="h-24 w-24 rounded-full object-cover md:col-span-2" src={form.photoUrl} />}<Field label="Staff photo"><input className="form-input mt-1" type="file" accept="image/*" onChange={(event) => upload(event.target.files?.[0])} /></Field>
            ...
            <Field label="Department"><select className="form-select mt-1" value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })}>{['GENERAL', 'CONSTRUCTION', 'REAL_ESTATE', 'MATERIAL_MANAGEMENT'].map((value) => <option value={value}>{value.replace(/_/g, ' ')}</option>)}</select></Field><Field label="Status"><select className="form-select mt-1" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>{['ACTIVE', 'INACTIVE', 'ON_LEAVE', 'TERMINATED'].map((value) => <option value={value}>{value.replace(/_/g, ' ')}</option>)}</select></Field>
            ...
            <Field label="Assigned project"><select className="form-select mt-1" value={form.assignedProjectId} onChange={(e) => setForm({ ...form, assignedProjectId: e.target.value })}><option value="">No project</option>{projects.map((project) => <option value={project.id}>{project.name}</option>)}</select></Field>
            ...
            {!editing && <><label className="flex items-center gap-2 md:col-span-2"><input type="checkbox" className="form-checkbox" checked={form.createAccount} onChange={(e) => setForm({ ...form, createAccount: e.target.checked })} />Create a login account now</label>{form.createAccount && <><Field label="Email" required><input type="email" className="form-input mt-1" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></Field><Field label="Role"><select className="form-select mt-1" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>{roles.map((role) => <option value={role.key}>{role.name}</option>)}</select></Field><Field label="Temporary password" required><input type="password" minLength={12} className="form-input mt-1" required value={form.temporaryPassword} onChange={(e) => setForm({ ...form, temporaryPassword: e.target.value })} /></Field></>}</>}
            <button disabled={saving} className="btn btn-primary md:col-span-2">{saving ? 'Saving…' : 'Save staff member'}</button>
        </form></Modal>
        <Modal title={selected ? `${selected.firstName} ${selected.lastName}` : 'Staff details'} open={Boolean(selected)} onClose={() => setSelected(null)} wide>{selected && <div className="space-y-6"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[['Department', selected.department], ['Position', selected.position], ['Hire date', shortDate(selected.hireDate)], ['Salary', money(selected.salary)]].map(([label, value]) => <div><p className="text-xs uppercase text-white-dark">{label}</p><p className="mt-1 font-bold">{value || '—'}</p></div>)}</div><div className="rounded-lg border border-white-light p-4 dark:border-[#191e3a]"><h3 className="font-bold">User account</h3>{selected.user ? <><p className="mt-2">{selected.user.email} · {selected.user.role}</p><div className="mt-4 flex flex-wrap gap-2"><button className="btn btn-sm btn-outline-warning" onClick={() => accountAction('status')}>{selected.user.isActive ? 'Deactivate' : 'Activate'}</button><button className="btn btn-sm btn-outline-primary" onClick={() => accountAction('email')}>Change email</button><button className="btn btn-sm btn-outline-danger" onClick={() => accountAction('password')}>Reset password</button></div></> : <button className="btn btn-primary mt-3" onClick={() => { setAccount({ email: '', role: 'STAFF', temporaryPassword: '' }); setAccountOpen(true); }}>Create login account</button>}</div><div><h3 className="mb-3 font-bold">Recent activity</h3>{!activity.length ? <p className="text-white-dark">No account activity recorded.</p> : <div className="max-h-64 space-y-2 overflow-y-auto">{activity.map((item) => <div className="rounded-md bg-gray-50 p-3 dark:bg-dark"><strong>{item.action}</strong> · {item.entity}<span className="float-right text-xs text-white-dark">{shortDate(item.createdAt)}</span><p className="text-xs text-white-dark">{item.details}</p></div>)}</div>}</div></div>}</Modal>
        <Modal title="Create staff login" open={accountOpen} onClose={() => setAccountOpen(false)}><form className="space-y-4" onSubmit={saveAccount}><Field label="Email" required><input className="form-input mt-1" type="email" required value={account.email} onChange={(e) => setAccount({ ...account, email: e.target.value })} /></Field><Field label="Role"><select className="form-select mt-1" value={account.role} onChange={(e) => setAccount({ ...account, role: e.target.value })}>{roles.map((role) => <option value={role.key}>{role.name}</option>)}</select></Field><Field label="Temporary password" required><input className="form-input mt-1" type="password" minLength={12} required value={account.temporaryPassword} onChange={(e) => setAccount({ ...account, temporaryPassword: e.target.value })} /></Field><button className="btn btn-primary w-full" disabled={saving}>Create account</button></form></Modal>
    </AppShell>;
};
