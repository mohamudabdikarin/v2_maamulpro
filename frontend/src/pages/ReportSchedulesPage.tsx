...
const ReportSchedulesPage = () => {
    const [reports, setReports] = useState<Report[]>([]); const [rows, setRows] = useState<Schedule[]>([]); const [entities, setEntities] = useState<any[]>([]); const [form, setForm] = useState<any>(empty); const [editing, setEditing] = useState<Schedule | null>(null); const [open, setOpen] = useState(false); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
    ...
    const remove = async (id: string) => { if (!window.confirm('Delete this report schedule?')) return; try { await api(`/api/reports/schedules/${id}`, { method: 'DELETE' }); load(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to delete schedule'); } };
