...
type Report = { id: string; title: string; workspace: string; description?: string };
type Schedule = { id: string; reportId: string; name: string; frequency: string; recipients?: string; filters?: string; nextRunAt?: string; isActive: boolean; lastSuccessAt?: string; lastFailureAt?: string; lastError?: string };
...
const ReportSchedulesPage = () => {
    ...
    const reportName = (id: string) => reports.find((report) => report.id === id)?.title || id;
    return <AppShell><PageHeader eyebrow="Automated reporting" title="Report schedules" description="Deliver selected reports to recipients on controlled weekly, monthly or yearly schedules." actions={<button className="btn btn-primary" onClick={() => { setEditing(null); setForm(empty); setOpen(true); }}>New schedule</button>} />
        ...
        {error && <ErrorAlert message={error} onRetry={load} />}{loading ? <div className="panel"><LoadingState /></div> : !rows.length ? <div className="panel"><EmptyState title="No scheduled reports" description="Create a schedule to automate report delivery." /></div> : <div className="panel overflow-hidden p-0"><div className="overflow-x-auto"><table className="table-hover"><thead><tr><th>Schedule</th><th>Report</th><th>Frequency</th><th>Recipients</th><th>Next run</th><th>Last delivery</th><th>Status</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td className="font-bold">{row.name}</td><td>{reportName(row.reportId)}</td><td>{row.frequency}</td><td className="max-w-xs truncate">{row.recipients || 'No recipients'}</td><td>{shortDate(row.nextRunAt)}</td><td title={row.lastError || ''}>{row.lastFailureAt && (!row.lastSuccessAt || new Date(row.lastFailureAt) > new Date(row.lastSuccessAt)) ? <span className="text-danger">Failed · {shortDate(row.lastFailureAt)}</span> : row.lastSuccessAt ? <span className="text-success">Delivered · {shortDate(row.lastSuccessAt)}</span> : 'Not run'}</td><td><StatusPill value={row.isActive ? 'ACTIVE' : 'INACTIVE'} /></td><td><div className="flex gap-2"><button className="btn btn-sm btn-outline-primary" onClick={() => edit(row)}>Edit</button><button className="btn btn-sm btn-outline-danger" onClick={() => setDeleteId(row.id)}>Delete</button></div></td></tr>)}</tbody></table></div></div>}
        <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Edit report schedule' : 'New report schedule'} wide><form className="grid gap-5 md:grid-cols-2" onSubmit={submit}>
            <Field label="Report" required><select className="form-select mt-1" required value={form.reportId} onChange={(e) => setForm({ ...form, reportId: e.target.value })}><option value="">Select report…</option>{reports.map((report) => <option value={report.id} key={report.id}>{report.title} · {report.workspace.replace(/_/g, ' ')}</option>)}</select></Field>
