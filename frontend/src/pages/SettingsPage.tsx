...
import { BookOpen, Building2, CreditCard, KeyRound, Palette, Tags, UserRound } from 'lucide-react';
...
const SettingsPage = () => {
    ...
    const save = async (event: FormEvent, type: 'company' | 'profile' | 'password') => {
        ...
        try {
            if (type === 'company' && settings) { const { companyName, logoUrl, companyEmail, companyPhone, companyAddress, companyDescription } = settings; await api('/api/settings', { method: 'PATCH', body: JSON.stringify({ companyName, logoUrl, companyEmail, companyPhone, companyAddress, companyDescription }) }); }
            else if (type === 'profile' && profile) { await api('/api/settings/profile', { method: 'PATCH', body: JSON.stringify({ name: profile.name, email: profile.email, avatarUrl: profile.avatarUrl }) }); await api('/api/settings/language', { method: 'PATCH', body: JSON.stringify({ language: profile.language }) }); }
            else {
    ...
    return <AppShell>
        ...
        <div className="panel overflow-hidden p-0">
            <div className="flex min-h-[620px] items-stretch">
                <aside className="w-64 shrink-0 border-r border-white-light bg-gray-50/50 p-4 dark:border-dark dark:bg-[#0e1726]"><div className="mb-5 px-3"><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Settings</p><p className="mt-1 text-sm text-white-dark">Company and personal controls</p></div><nav className="flex flex-col gap-1" aria-label="Settings navigation">{visibleSections.map((section) => { const Icon = section.icon; return <button key={section.id} type="button" onClick={() => setActiveSection(section.id)} className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-left text-sm font-semibold transition ${activeSection === section.id ? 'bg-primary-light text-primary' : 'text-white-dark hover:bg-white dark:hover:bg-black'}`} aria-current={activeSection === section.id ? 'page' : undefined}><Icon size={18} />{section.label}</button>; })}</nav></aside>
