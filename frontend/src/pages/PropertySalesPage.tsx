...
const PropertySalesPage = () => {
    ...
    return <AppShell><PageHeader eyebrow="Real estate revenue" title="Property sales" description="Sales pipeline, settlement status, collections and outstanding balances." actions={hasPermission('deals.create') ? <Link className="btn btn-primary" to="/app/real-estate/deals/new">New sale</Link> : undefined} />
        ...
        {state.error && <ErrorAlert message={state.error} onRetry={state.reload} />}{state.loading ? <div className="panel"><LoadingState /></div> : !rows.length ? <div className="panel"><EmptyState title="No property sales" description="Create a sale deal to begin tracking settlement." action={hasPermission('deals.create') ? <Link className="btn btn-primary" to="/app/real-estate/deals/new">New sale</Link> : undefined} /></div> : <div className="panel overflow-hidden p-0"><div className="overflow-x-auto"><table className="table-hover"><thead><tr><th>Property</th><th>Buyer</th><th>Closed</th><th>Total</th><th>Paid</th><th>Balance</th><th>Status</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{row.property?.title}</td><td>{row.client?.name}</td><td>{shortDate(row.closedAt)}</td><td>{money(row.totalAmount)}</td><td className="text-success">{money(row.paidAmount)}</td><td className="text-danger">{money(Number(row.totalAmount) - Number(row.paidAmount))}</td><td><StatusPill value={row.paymentStatus} /></td><td><Link className="text-primary" to={`/app/real-estate/deals/${row.id}`}>Open</Link></td></tr>)}</tbody></table></div></div>}
    </AppShell>;
};
