...
const emptyWorker = { firstName: '', lastName: '', phone: '', position: '', workerTypeId: '', assignedProjectId: '', notes: '' };
...
const ManpowerPage = () => {
    ...
    const openCreate = () => { setEditing(null); setForm(emptyWorker); setModal(true); };
    const openEdit = (worker: any) => { setEditing(worker); setForm({ ...emptyWorker, firstName: worker.firstName, lastName: worker.lastName, phone: worker.phone || '', position: worker.position || '', workerTypeId: worker.workerTypeId || '', assignedProjectId: worker.assignedProjectId || '', notes: worker.notes || '' }); setModal(true); };
    const saveWorker = async (event: FormEvent) => {
        ...
        try {
            ...
            await api(editing ? `/api/construction/manpower/workers/${editing.id}` : '/api/construction/manpower/workers', { method: editing ? 'PATCH' : 'POST', silent: true, body: JSON.stringify(payload) });
            setModal(false); toast.success(editing ? 'Worker updated.' : 'Worker registered.'); load();
    ...
    return <AppShell>
        <PageHeader eyebrow="Construction workforce" title="Manpower dashboard" description="Worker assignments, labor classifications, operational costs and ledger activity." actions={<>
            ...
            {canCreate && <button className="btn btn-primary" onClick={openCreate}>Register worker</button>}
        </>} />
        ...
        <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit worker' : 'Register worker'}><form className="grid gap-4 sm:grid-cols-2" onSubmit={saveWorker}>
